...
import (
	...
	"personal_bot/backend/api/handler"
	"personal_bot/backend/app"
	programnames "personal_bot/backend/app/program_names"
	"personal_bot/backend/infrastructure/persistence/repository"
	"personal_bot/backend/internal/core/program"
	cryptostates "personal_bot/backend/internal/services/crypto_states"
	...
	tradingstrategy "personal_bot/backend/internal/services/trading/strategy"
	"personal_bot/backend/internal/services/wallet"
	datastream "personal_bot/backend/internal/solana/monitoring/data_stream"
	pumpfunamm "personal_bot/backend/internal/solana/programs/pumpfun/pumpfun_amm"
	pumpfunnative "personal_bot/backend/internal/solana/programs/pumpfun/pumpfun_native"
	...
	"personal_bot/backend/internal/services/transaction"
...
func WireServicesAndLaunchServer(mainCtx context.Context, db *sql.DB) (*http.Server, ShutdownServices) {
	...
	taskService := taskservice.NewTaskService(taskRepo, taskSubhub, taskManager, positionService, rpcGroupService)
	deps.TaskService = taskService

	tradingStrategy := tradingstrategy.New(taskService, posSubhub, positionService, strategySubHub, taskSubhub, rpcGroupService)
