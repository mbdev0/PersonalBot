...
import (
	...
	pumpInstructions "personal_bot/backend/internal/solana/programs/pumpfun/pumpfun_native/instructions"
	pumpfunmodels "personal_bot/backend/internal/solana/programs/pumpfun/pumpfun_native/models"
	"personal_bot/backend/internal/solana/programs/pumpfun/pumpfun_native/pda"
...
func (bt *Transaction) SendTransaction(ctx context.Context) error {
	...
	sig, err := transaction.SendTx(ctx, rpcClient, bt.transaction)
	if err != nil {
		return err
	}

	bt.signature = sig
	bt.Publisher.PublishMessage(bt.BuyTask, fmt.Sprintf("Tx Sent: %s", sig))
	return nil
}
...
func (bt *Transaction) UpdatePosition(ctx context.Context) (tokenAmount, solAmount float64, pos *positionmodels.Position, err error) {
	...
	return float64(tradeEvent.TokenAmount), solAmnt, nil, nil
}

func (bt *Transaction) extractTokenAndSolFromTx(ctx context.Context, signature solana.Signature) (solAmount float64, tradeEvent pumpfunmodels.TradeEvent, err error) {
